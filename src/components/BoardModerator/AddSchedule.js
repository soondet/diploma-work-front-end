...
class AddSchedule extends Component {
  ...
  componentDidMount = () => {
    SeatPlaceService.getByBusModelId(JSON.parse(localStorage.getItem("user")).bus.busModel.id).then((response) => {
      this.setState({
  ...
  showRoutes = () => {
    ...
    SequenceService.getSequenceRouteIdsByAddressIds(this.state.addressesChoosedIds).then((response) => {
      response.data.forEach((e) => {
        SequenceService.getSequenceByRouteId(e).then((response) => {
          this.setState({
            routeAddresses: [...this.state.routeAddresses, {
  ...
  onFinishAddingSchedule = (values) => {
    ScheduleService.createSchedule(
      ...
      JSON.parse(localStorage.getItem("user")).bus.busModel.seatNumber - values.seats.length,
    ).then(() => {
      ScheduleService.getOneSchedule(this.state.choosedRoute.id, JSON.parse(localStorage.getItem("user")).bus.id, moment(this.state.datePickedToAdd).format('YYYY-MM-DD')).then((response) => {
                ...
                >
                  <Row>
                    <Col className="schedule-ticket">
                      ...
                      <p>Адреса: {item.addresses.map((e => e.address.addressName)) + " "}</p>
                    </Col>
                  </Row>
                  <Divider />
                  <Button
